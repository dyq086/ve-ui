...
export default class MeScroll {
  constructor(mescrollId, options) {
    ...
    setTimeout(()=> {
      ...
      if (this.optDown.auto) {
        //默认true 以下拉刷新的方式自动加载第一页数据
        if (this.optDown.autoShowLoading) {
          this.triggerDownScroll(); //显示下拉进度,执行下拉回调
        } else {
          this.optDown.callback && this.optDown.callback(this); //不显示下拉进度,直接执行下拉回调
        }
      }

      this.optUp.auto && this.triggerUpScroll(); //默认false
    }, 30); //需让me.optDown.inited和me.optUp.inited先执行
  }


/*配置参数:下拉刷新*/
extendDownScroll (optDown) {
  //是否为ios设备;
  ...
  MeScroll.extend(optDown, {
    ...
    minAngle: 45, //触发下拉最少要偏移的角度(滑动的轨迹与水平线的锐角值),取值区间  [0,90];默认45度,即向下滑动的角度大于45度(方位角为45°~145°及225°~315°)则触发下拉;而小于45度,将不触发下拉,避免与左右滑动的轮播等组件冲突;<br/>注意:没有必要配置超出[0,90]区间的值,否则角度限制无效; 因为假设配置30, 生效的方位角就是30°到160° 和 210°到330°的范围;
...
extendUpScroll (optUp) {
  ...
  MeScroll.extend(optUp, {
    ...
    onScroll: null, //列表滑动监听,默认null; 例如 onScroll(mescroll, y){ }; //y为列表当前滚动条的位置
    scrollbar: {
...
initDownScroll() {
  ...
  //可配置不使用下拉刷新
  if (this.optDown.use == false) return;
  //具体参数配置
  ...
  this.downwarp.innerHTML =
    '<div class="downwarp-content">' + this.optDown.htmlContent + "</div>";
  this.scrollDom.insertBefore(this.downwarp, this.scrollDom.firstChild);

  //鼠标或手指的按下事件
  this.scrollDom.addEventListener("touchstart", touchstartEvent.bind(this)); //移动端手指事件
  this.scrollDom.addEventListener("mousedown", touchstartEvent.bind(this)); //PC端鼠标事件
  function touchstartEvent(e) {
    if (this.isScrollTo) e.preventDefault(); //如果列表执行滑动事件,则阻止事件,优先执行scrollTo方法
    this.startTop = this.scrollDom.scrollTop; //此时列表到顶部的距离,鼠标或手指离开时制空,避免直接触发move事件;
    if (this.optDown.mustToTop) {
      this.startY = e.touches ? e.touches[0].pageY : e.clientY; //如果列表必须滑动到顶部才能下拉,则记录此时第一个手指距离列表顶部的距离
    }
  }

  //鼠标或手指的滑动事件
  this.scrollDom.addEventListener("touchmove", touchmoveEvent.bind(this)); //移动端手指事件
  this.scrollDom.addEventListener("mousemove", touchmoveEvent.bind(this)); //PC端鼠标事件
  function touchmoveEvent(e) {
    ...
    if (
      ...
      //是否列表必须滑动到顶部才能下拉
      if (this.optDown.mustToTop && this.startTop > 0) return;
      ...
      if (!this.preX) this.preX = curX; //设置上次移动的距离x
      if (!this.preY) this.preY = curY; //设置上次移动的距离y

      //计算两点之间的角度
      let x = Math.abs(this.preX - curX);
      let y = Math.abs(this.preY - curY);
      let z = Math.sqrt(x * x + y * y);

      let diff = curY - this.preY; //和上次比,移动的距离 (大于0向下,小于0向上)
      this.preX = curX; //记录本次curX的值
      this.preY = curY; //记录本次curY的值

      if (z != 0) {
        let angle = Math.asin(y / z) / Math.PI * 180; //角度区间 [0,90]
        if (angle < this.optDown.minAngle) return; //如果小于配置的角度,则不往下执行 -- 1.1.6版本加入的配置
      }

      if (!this.startY && !this.optDown.mustToTop) this.startY = curY; //记住列表滚动到0的位置时,手指距离顶部的距离,在touchend重置
      let moveY = curY - this.startY; //和起点比,移动的距离,大于0向下拉
      ...
      if (moveY > 0) {
        ...
        if (this.downHight < this.optDown.offset) {
          if (this.movetype != 1) {
            ...
            this.scrollDom.style.webkitOverflowScrolling = "auto"; //取消列表回弹效果,避免与下面me.downwarp.style.height混合,而导致界面抖动闪屏
        ...
        let rate = this.downHight / this.optDown.offset; //下拉区域当前高度与指定距离的比值
  ...
  //鼠标或手指的离开事件
  this.scrollDom.addEventListener("touchend", touchendEvent.bind(this)); //移动端手指事件
  this.scrollDom.addEventListener("mouseup", touchendEvent.bind(this)); //PC端鼠标抬起事件
  this.scrollDom.addEventListener("mouseleave", touchendEvent.bind(this)); //PC端鼠标离开事件
  ...
  setTimeout(()=> {
    //待主线程执行完毕再执行,避免new MeScroll未初始化,在回调获取不到mescroll的实例
    this.optDown.inited(this, this.downwarp);
  }, 0);
};
...
initUpScroll() {
  //配置参数
  this.optUp = this.options.up || {};
  //可配置不使用上拉加载
  if (this.optUp.use == false) return;
  //具体参数配置
  this.extendUpScroll(this.optUp);

  //自定义滚动条 (默认只在PC端设置)
  if (this.optUp.scrollbar.use)
    this.scrollDom.classList.add(this.optUp.scrollbar.barClass);
  ...
  //滚动监听
  this.scrollDom.addEventListener("scroll", function() {
    //列表内容顶部卷去的高度(含列表边框)
    ...
    if (
      ...
      if (!this.optUp.isLock) {
        let toBottom =
          this.scrollDom.scrollHeight - this.scrollDom.clientHeight - scrollTop; //滚动条距离底部的距离
      ...
      //顶部按钮的显示隐藏
      if (this.optUp.toTop.src) {
        if (scrollTop >= this.optUp.toTop.offset) {
          this.showTopBtn();
        } else {
          this.hideTopBtn();
        }
    ...
    //滑动监听
    this.optUp.onScroll && this.optUp.onScroll(this, scrollTop);
  }.bind(this));

  //初始化完毕的回调
  setTimeout(()=> {
    //待主线程执行完毕再执行,避免new MeScroll未初始化,在回调获取不到mescroll的实例
    this.optUp.inited(this, this.upwarp);
  }, 0);
};
...
/*清空上拉加载的数据列表*/
clearDataList() {
  let listId = this.optUp.clearId || this.optUp.clearEmptyId; //优先使用clearId
  if (listId) {
 ...
 */
endSuccess(dataSize, systime) {
  //结束下拉刷新
  if (this.isDownScrolling) this.endDownScroll();

  //结束上拉加载
  if (this.optUp.use) {
    let pageNum = this.optUp.page.num; //当前页码
    let pageSize = this.optUp.page.size; //每页长度
...
showTopBtn() {
  if (!this.topBtnShow) {
    ...
    let optTop = this.optUp.toTop; //回到顶部的配置
    if (this.toTopBtn == null) {
      //未加入按钮,则加入
      this.toTopBtn = document.createElement("img");
      this.toTopBtn.className = optTop.warpClass;
    ...
    //显示--淡入动画
    this.toTopBtn.classList.remove(optTop.hideClass);
    this.toTopBtn.classList.add(optTop.showClass);
  }
};
/*隐藏回到顶部的按钮*/
hideTopBtn() {
...
scrollTo(y, t) {
  ...
  let rate = 20; //周期 20ms
  let count = t / rate; //次数
  let maxY = this.scrollDom.scrollHeight - this.scrollDom.clientHeight; //y的最大值
  if (y < 0) y = 0; //不可小于0
  if (y > maxY) y = maxY; //不可超过最大值
  let diff = this.scrollDom.scrollTop - y; //差值 (可能为负值)
  if (diff == 0) return;
  let step = diff / count; //步长
  this.isScrollTo = true; //标记在滑动中,阻止列表的触摸事件
  let i = 0; //计数
  let scrollTimer = window.setInterval(()=> {
    if (i < count) {
      if (i == count - 1) {
        this.scrollDom.scrollTop = y; //最后一次直接设置y,避免计算误差
      } else {
